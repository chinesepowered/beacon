# Beacon — Convex All Gas Hackathon build plan

> **One line:** Lost-pet finder that crawls shelter pages on a schedule, AI-matches found listings to your pet, emails every shelter and vet, and maps sightings live.
>
> **Repo:** `hack-gas1` · **AgentMail account:** A · **Event:** Convex All Gas Hackathon · **Deadline:** September 22, 2026, 12:00 PM PT · **Stack:** Convex (backend + static hosting on `*.convex.site`), React + Vite + Tailwind, any OpenAI-compatible LLM endpoint (env-configured; described as "OpenAI" in judge-facing docs), Firecrawl, AgentMail.
>
...
## 3. Product spec — Beacon (lost pet finder)

### 3.1 The pitch
When a pet goes missing, the owner has hours of panic and no system: shelters post "found animals" on a dozen different web pages, and every vet and rescue has to be emailed by hand. **Beacon turns a lost-pet report into an always-on search:** it crawls every local shelter/found-pet page on a schedule, uses AI (vision when available) to match found listings to your pet, emails every shelter and vet in range with the flyer, and puts every reply and sighting on a live map. Emotional payoff: the moment a match card appears with a photo side-by-side and "92% — same white chest patch". Visual payoff: a live map with pins dropping in.

### 3.2 Core loop
Owner creates a case (photo, details, last-seen location) → **Firecrawl** `search` discovers local shelters / lost-and-found pages and a **cron** re-scrapes each source with JSON extraction → **LLM** scores each new found-listing against open cases (text + optional vision) → high scores become **matches** the owner sees live → on case creation **AgentMail** creates a case inbox, emails the shelter/vet contact list with the flyer, and every **reply** (webhook) is classified as sighting / no-match / question, sightings become map pins → the public flyer page (unauthenticated slug) lets a finder report a sighting by form or by emailing the case inbox with a photo.

### 3.3 Data model (`convex/schema.ts`)
- `cases`: ownerId, slug (unguessable), status (`open|found|closed`), petName, species, breed, colors[], distinguishingMarks, sizeKg?, photoIds[] (storage), lastSeenLat/Lng, lastSeenAddress, lastSeenAt, radiusKm, caseCode (short unique code for email routing, see §4.3), createdAt. Index by ownerId, by slug, by caseCode, by status.
- `sources`: caseId?, (or regionKey), url, kind (`shelter|lostfound|vet`), name, orgEmail?, lastScrapedAt, lastHash, status. Index by caseId, by url.
- `listings`: sourceId, listingUrl (unique key), species, description, colors[], locationText, foundAt?, imageUrl?, rawExcerpt, extracted (json), fetchedAt. Index by listingUrl, by sourceId.
- `matches`: caseId, listingId, score (0–100), reasons[], visionUsed (bool), model, status (`new|dismissed|confirmed`), createdAt. Index by caseId+status, by listingId.
- `contacts`: caseId, orgName, email, kind, sourceUrl, emailStatus (`queued|sent|delivered|bounced|replied`), lastMessageId?. Index by caseId.
- `messages`: caseId, direction (`in|out`), messageId, threadId, from (redacted in logs), subject, extractedText, fullText, attachmentIds[] (storage), classification (`sighting|no_match|question|auto_reply|other`)?, extractedLat/Lng?, receivedAt. Index by messageId, by caseId.
- `sightings`: caseId, lat, lng, note, photoId?, source (`form|email`), messageId?, reportedAt. Index by caseId.

### 3.4 Convex functions
- `cases.create` (mutation → schedules `pipeline.kickoff`), `cases.get`, `cases.listMine`, `cases.publicBySlug` (no auth; returns pet info + sightings only), `cases.markFound`.
- `pipeline.kickoff` (internalAction): geocode address via Firecrawl scrape of a geocoder page **or** browser geolocation from the client (prefer client geolocation + manual pin to avoid a geocoding dependency); discover sources; assign caseCode; email contacts; schedule first scrape.
- `crawl.discoverSources` (internalAction): Firecrawl `search` for `"<city> animal shelter found animals"`, `"<city> lost and found pets"`, `"<city> veterinary clinic"`; LLM picks the top ~10 relevant URLs + extracts org email if present on the page → insert `sources`/`contacts`.
- `crawl.scrapeSource` (internalAction): Firecrawl `scrape` with JSON schema `{ listings: [{ species, description, colors, location, foundDate, imageUrl, url }] }`; upsert `listings` by URL; for each new listing schedule `ai.scoreListing`.
- `crons.ts`: every 30 min `crawl.sweep` scrapes all sources for open cases (skip if `lastScrapedAt` < 25 min; use Firecrawl `maxAge` caching).
- `ai.scoreListing` (internalAction): for each open case within radius/species, `extract()` a `{score, reasons[]}`; if `LLM_VISION_MODEL` set and both images exist, run a vision compare and blend; insert `matches` where score ≥ 60.
- `mail.sendFlyers` (internalAction): render flyer HTML (photo via public storage URL, description, public slug link, the app inbox address as reply-to and the case code in the subject) and `sendMail()` to each contact.
- `http.ts` webhook → `mail.ingest` → `ai.classifyInbound` (`{classification, locationText?, lat?, lng?, summary}`) → if sighting, insert `sightings`.
- `sightings.reportPublic` (mutation, no auth, rate-limited by slug).

### 3.5 Integration specifics
- **Firecrawl:** `search` for discovery; `scrape` with JSON extraction for listings; `maxAge` to keep credits down; store `rawExcerpt` (first 1500 chars markdown) for the "source" drawer.
- **LLM:** scoring prompt gets case description + listing; return `score`, `reasons` (max 3 short phrases like "same breed", "found 2 km away, 1 day later", "white chest patch mentioned"). Vision optional.
- **AgentMail:** single app inbox (§4.3), case code `[BCN-xxxx]` in every subject; flyer email; replies routed by thread id / subject code; attachments (photos) downloaded into storage and, if vision is enabled, scored against the case photo as a match.

### 3.6 UI
- **/** landing: one big button "My pet is missing", plus "I found a pet" (opens the public sighting form by searching active cases near you — stretch; MVP just links to flyer pages).
- **/new** 3-step wizard: photo (drag/drop, storage upload) → details → map pin (Leaflet/MapLibre with OSM tiles, click to set last-seen; radius slider).
- **/case/:id** the war room, three panes: **Map** (last-seen pin, radius circle, sighting pins with photos, match pins from listings with locations) · **Live feed** (rows animate in: "Scanned Humane Society — 14 listings, 1 possible match", "Emailed 9 shelters", "Reply from [shelter]: no match yet") · **Matches** (side-by-side photo cards, score ring, reasons, "This is them!" / "Not them" — confirming flips the case to Found with a confetti moment and emails all contacts "found, thank you").
- **/p/:slug** public flyer: printable, QR code (generate client-side), big photo, "I saw this pet" form → sighting, and the case email address.
- Look: warm, high-contrast, large photo. Motion: pins drop; new match card slides in with a pulse.

### 3.7 Demo storyboard (≤ 3:00)
1. (0:00) "Milo went missing last night." Open the live URL, click *My pet is missing*, drop his photo, pin the park. (0:30)
2. War room opens empty; feed starts filling: "Found 8 shelters near you" (Firecrawl rows with source links), "Emailed 8 shelters" — open one contact's sent email preview. (1:00)
3. Show the cron sweep: click *Scan now* on a source; listings appear; a match card slides in with photo side-by-side, 87%, reasons. (1:30)
4. Phone in frame: reply to the shelter email (it went to your own inbox via the override) "We have a beagle brought in this morning near Victoria Park, photo attached." Reply lands in feed within seconds, classified *sighting*, a pin drops on the map, photo shows. (2:20)
5. Click "This is them!" → confetti, status Found, "8 shelters notified". Show public flyer page + URL. (2:50)

### 3.8 Fixtures & test plan
- `convex/seed.ts`: one demo case (use a stock dog photo committed under `public/demo/`), 3 sources pointing at real public shelter pages in the human's city, and 2 fake listings so a match exists on first load.
- Test the webhook with a hand-sent email from the human's mailbox to the app inbox with the case code in the subject before filming.

### 3.9 Stretch
Vision matching from emailed photos; "found a pet" reverse search; SMS-less push via email digest at 8am (cron).

### 3.10 Risks
Geocoding — avoid by using client geolocation + map click. Shelter pages vary wildly — JSON extraction with a permissive schema and let the LLM score from free text. Keep species filter strict to avoid silly matches in the demo.
